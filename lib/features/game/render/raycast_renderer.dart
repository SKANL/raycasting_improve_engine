...
class RaycastRenderer extends PositionComponent
  ...
  void render(Canvas canvas) {
    ...
    try {
      ...
      final pickupsToDraw =
          <({v64.Vector2 pos, PickupComponent pickup, double distSq})>[];
      ...
      if (_latestState.spriteAtlas != null) {
        for (final entity in _latestState.entities) {
          ...
          if (t != null && srcRect != null) {
            ...
            final camDot =
                _cachedDirX * spriteDelta.x + _cachedDirY * spriteDelta.y;
            ...
            final perpAbs =
                (_cachedDirX * spriteDelta.y - _cachedDirY * spriteDelta.x)
                    .abs();
  ...
  void _bakeWeaponCarry(Canvas canvas, String id, double w, double h) {
    switch (id) {
      case 'pistol':
        _vmPistolCarry(canvas, w, h, bounce: false);
      case 'bounce_pistol':
        _vmPistolCarry(canvas, w, h, bounce: true);
      case 'shotgun':
        _vmShotgunCarry(canvas, w, h);
      case 'rifle':
        _vmRifleCarry(canvas, w, h, bounce: false);
      case 'bounce_rifle':
        _vmRifleCarry(canvas, w, h, bounce: true);
    }
  }
  ...
  void _bakeWeaponAim(Canvas canvas, String id, double w, double h) {
    switch (id) {
      case 'pistol':
        _vmPistolAim(canvas, w, h, bounce: false);
      case 'bounce_pistol':
        _vmPistolAim(canvas, w, h, bounce: true);
      case 'shotgun':
        _vmShotgunAim(canvas, w, h);
      case 'rifle':
        _vmRifleAim(canvas, w, h, bounce: false);
      case 'bounce_rifle':
        _vmRifleAim(canvas, w, h, bounce: true);
    }
  }
  ...
  void _vmPistolCarry(
    ...
    final Color shadow = bounce
        ? const Color(0xFF0A1A16)
        : const Color(0xFF1A1A1A);
    final Color body = bounce
        ? const Color(0xFF1E4840)
        : const Color(0xFF505050);
    final Color bright = bounce
        ? const Color(0xFF2E6858)
        : const Color(0xFF787878);
    final Color accent = bounce
        ? const Color(0xFF1DFFB4)
    ...
    canvas.drawLine(
      Offset(w * 0.446, h * 0.076),
      Offset(w * 0.554, h * 0.076),
    ...
    canvas.drawRect(
      Rect.fromLTWH(w * 0.450, h * 0.078, w * 0.010, h * 0.020),
      f(const Color(0xFF606060)),
    );
    canvas.drawCircle(
      Offset(w * 0.455, h * 0.093),
      h * 0.005,
      f(const Color(0xFFFFFFFF)),
    );
    canvas.drawRect(
      Rect.fromLTWH(w * 0.540, h * 0.078, w * 0.010, h * 0.020),
      f(const Color(0xFF606060)),
    );
    canvas.drawCircle(
      Offset(w * 0.545, h * 0.093),
      h * 0.005,
      f(const Color(0xFFFFFFFF)),
    );
    ...
    if (bounce) {
      canvas.drawLine(
        Offset(w * 0.447, h * 0.077),
        Offset(w * 0.447, h * 0.577),
        f(accent)
          ..strokeWidth = 1.5
    ...
    canvas.drawRect(
      Rect.fromLTWH(w * 0.476, h * 0.030, w * 0.028, h * 0.052),
      f(const Color(0xFF2A2A2A)),
    );
    // Bore hole
    canvas.drawCircle(
      Offset(w * 0.490, h * 0.042),
      h * 0.011,
      f(const Color(0xFF060606)),
    );
    // Front sight blade on top of barrel stub
    canvas.drawRect(
      Rect.fromLTWH(w * 0.483, h * 0.072, w * 0.012, h * 0.014),
    ...
    canvas.drawRect(frameR, mgh(frameR, shadow, body, shadow));
    // Picatinny rail groove at bottom of frame
    canvas.drawRect(
      Rect.fromLTWH(w * 0.444, h * 0.660, w * 0.110, h * 0.007),
    ...
    // Trigger
    canvas.drawLine(
      Offset(w * 0.492, h * 0.666),
      Offset(w * 0.488, h * 0.704),
    ...
    for (var gy = 0; gy < 4; gy++) {
      for (var gx = 0; gx < 3; gx++) {
        canvas.drawCircle(
          ...
          h * 0.004,
          f(const Color(0xFF2E2E2E)),
        );
      }
    }
    // Left bright bevel (shows grip thickness)
    canvas.drawLine(
      Offset(w * 0.442, h * 0.710),
      Offset(w * 0.440, h * 1.050),
      f(body)..strokeWidth = 3.0,
    );
    // Magazine base plate
    canvas.drawRect(
      Rect.fromLTWH(w * 0.442, h * 0.994, w * 0.100, h * 0.018),
      f(const Color(0xFF888888)),
    );
    if (bounce) {
      canvas.drawRect(
        Rect.fromLTWH(w * 0.444, h * 0.810, w * 0.098, h * 0.014),
  ...
  void _vmShotgunCarry(Canvas canvas, double w, double h) {
    ...
    canvas.drawRect(
      Rect.fromLTWH(w * 0.498, h * 0.055, w * 0.004, h * 0.466),
      f(const Color(0xFFAAAAAA)),
    );
    // Muzzle crowns — ovals at top
    for (final bx in [barXL, barXR]) {
      canvas.drawOval(
        Rect.fromLTWH(bx - w * 0.002, h * 0.044, barW + w * 0.004, h * 0.018),
        f(const Color(0xFF2A2A2A))
          ..style = PaintingStyle.stroke
          ..strokeWidth = 2.0,
      );
      canvas.drawCircle(
        Offset(bx + barW / 2, h * 0.053),
        h * 0.008,
    ...
    canvas.drawRect(
      Rect.fromLTWH(w * 0.474, h * 0.298, w * 0.052, h * 0.005),
    ...
    canvas.drawRect(
      Rect.fromLTWH(w * 0.462, h * 0.434, w * 0.076, h * 0.007),
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(recR, const Radius.circular(4)),
      mgv(recR, recDark, recMid, recDark),
    );
    canvas.drawRect(
      Rect.fromLTWH(w * 0.444, h * 0.520, w * 0.112, h * 0.008),
      f(const Color(0x55FFFFFF)),
    );
    // Bolt handle marker
    canvas.drawRect(
      Rect.fromLTWH(w * 0.528, h * 0.542, w * 0.022, h * 0.030),
      f(const Color(0xFF383838)),
    );
    canvas.drawCircle(
      Offset(w * 0.498, h * 0.572),
      h * 0.008,
      f(const Color(0xFF111111)),
    );
    ...
    canvas.drawLine(
      Offset(w * 0.494, h * 0.634),
      Offset(w * 0.491, h * 0.666),
      f(const Color(0xFF555555))..strokeWidth = w * 0.006,
    );
    ...
    // Left bright bevel
    canvas.drawLine(
      Offset(w * 0.446, h * 0.680),
      Offset(w * 0.442, h * 1.050),
  ...
  void _vmRifleCarry(
    ...
    final Color shadow = bounce
        ...
        : const Color(0xFF1A1A1A);
    final Color body = bounce
        ? const Color(0xFF1A4048)
        : const Color(0xFF4E4E4E);
    final Color bright = bounce
        ? const Color(0xFF2A6070)
        : const Color(0xFF747474);
    final Color accent = bounce
        ? const Color(0xFF22D4FF)
        : const Color(0xFF888888);
    ...
    canvas.drawRect(
      Rect.fromLTWH(w * 0.484, h * 0.040, w * 0.030, h * 0.018),
    ...
    canvas.drawRect(barR, mgh(barR, shadow, const Color(0xFF3A3A3A), shadow));
    canvas.drawLine(
      Offset(w * 0.488, h * 0.056),
      Offset(w * 0.488, h * 0.216),
      f(const Color(0x66FFFFFF))..strokeWidth = 1.5,
    );
    // Bore
    canvas.drawCircle(
      Offset(w * 0.499, h * 0.066),
      h * 0.010,
      f(const Color(0xFF060606)),
    );
    // Gas block (small lump ~40% down exposed barrel)
    canvas.drawRect(
      Rect.fromLTWH(w * 0.484, h * 0.116, w * 0.030, h * 0.034),
      f(const Color(0xFF303030)),
    );
    // Gas tube runs alongside barrel
    canvas.drawRect(
      Rect.fromLTWH(w * 0.512, h * 0.120, w * 0.008, h * 0.100),
      f(const Color(0xFF252525)),
    );
    // Bounce accent
    if (bounce) {
      canvas.drawLine(
        Offset(w * 0.510, h * 0.056),
        Offset(w * 0.510, h * 0.216),
        f(accent)
          ..strokeWidth = 1.2
          ..blendMode = BlendMode.plus
          ..color = accent.withAlpha(80),
    ...
    canvas.drawRect(
      Rect.fromLTWH(w * 0.492, h * 0.044, w * 0.006, h * 0.016),
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(hgR, const Radius.circular(4)),
      mgh(hgR, shadow, body, shadow),
    );
    canvas.drawRect(
      Rect.fromLTWH(w * 0.444, h * 0.214, w * 0.112, h * 0.008),
      f(const Color(0x55FFFFFF)),
    );
    ...
    // Bottom rail line
    canvas.drawRect(
      Rect.fromLTWH(w * 0.444, h * 0.528, w * 0.112, h * 0.008),
    ...
    if (!bounce) {
      ...
      canvas.drawRect(chR, mgh(chR, shadow, bright, shadow));
      canvas.drawRect(
        Rect.fromLTWH(w * 0.446, h * 0.530, w * 0.140, h * 0.005),
        f(const Color(0x44FFFFFF)),
      );
      // Rear aperture
      canvas.drawRect(
        Rect.fromLTWH(w * 0.549, h * 0.534, w * 0.008, h * 0.015),
        f(const Color(0xFF111111)),
      );
    } else {
      // Scope
      ...
      canvas.drawRRect(
        RRect.fromRectAndRadius(scopeR, const Radius.circular(5)),
        mgh(scopeR, shadow, const Color(0xFF1A3040), shadow),
      );
      // Rear lens with subtle glow
      canvas.drawOval(
        Rect.fromLTWH(w * 0.442, h * 0.532, w * 0.036, h * 0.038),
        f(const Color(0xFF0A1824)),
      );
      canvas.drawOval(
        Rect.fromLTWH(w * 0.446, h * 0.536, w * 0.028, h * 0.030),
        f(accent)
          ..blendMode = BlendMode.plus
      ...
      canvas.drawRect(
        Rect.fromLTWH(w * 0.496, h * 0.524, w * 0.014, h * 0.012),
        f(const Color(0xFF303030)),
      );
    }
    ...
    canvas.drawRect(
      Rect.fromLTWH(w * 0.444, h * 0.564, w * 0.148, h * 0.006),
      f(const Color(0x44FFFFFF)),
    );
    // Ejection port — large and clear
    canvas.drawRect(
      Rect.fromLTWH(w * 0.546, h * 0.578, w * 0.044, h * 0.056),
      f(const Color(0xFF080808)),
    );
    canvas.drawRect(
      Rect.fromLTWH(w * 0.550, h * 0.590, w * 0.028, h * 0.024),
      f(const Color(0xFF7A5510)),
    );
    // Charging handle
    canvas.drawRect(
      Rect.fromLTWH(w * 0.444, h * 0.568, w * 0.016, h * 0.044),
      f(const Color(0xFF282828)),
    );
    if (bounce) {
      canvas.drawLine(
        Offset(w * 0.445, h * 0.565),
        Offset(w * 0.591, h * 0.565),
        f(accent)
          ..strokeWidth = 1.2
          ..blendMode = BlendMode.plus
    ...
    canvas.drawRRect(
      ...
      mgv(
        Rect.fromLTWH(w * 0.469, h * 0.704, w * 0.060, h * 0.226),
        shadow,
        bright,
        shadow,
      ),
    ...
    // Magazine top clamp
    canvas.drawRect(
      Rect.fromLTWH(w * 0.469, h * 0.704, w * 0.060, h * 0.010),
      f(bounce ? accent.withAlpha(180) : const Color(0xFF606060)),
    );
    // Magazine center seam
    canvas.drawLine(
      Offset(w * 0.499, h * 0.712),
      Offset(w * 0.499, h * 0.928),
    ...
    canvas.drawLine(
      Offset(w * 0.554, h * 0.654),
      Offset(w * 0.554, h * 0.860),
    ...
    canvas.drawLine(
      Offset(w * 0.514, h * 0.726),
      Offset(w * 0.510, h * 0.760),
      f(const Color(0xFF555555))..strokeWidth = w * 0.006,
    );
    ...
    canvas.drawRect(
      Rect.fromLTWH(w * 0.370, h * 0.576, w * 0.076, h * 0.020),
      f(const Color(0xFF282828)),
    );
    // Stock cheekweld
    canvas.drawRect(
      Rect.fromLTWH(w * 0.304, h * 0.558, w * 0.146, h * 0.028),
      f(body),
    );
    canvas.drawRect(
      Rect.fromLTWH(w * 0.304, h * 0.558, w * 0.146, h * 0.005),
      f(const Color(0x44FFFFFF)),
    );
    if (bounce) {
      canvas.drawRect(
        Rect.fromLTWH(w * 0.304, h * 0.578, w * 0.146, h * 0.010),
  ...
  void _vmPistolAim(Canvas canvas, double w, double h, {required bool bounce}) {
    ...
    final Color bodyDark = bounce
        ? const Color(0xFF0A1E1A)
        : const Color(0xFF181818);
    final Color bodyMid = bounce
        ? const Color(0xFF1A4A3A)
        : const Color(0xFF3C3C3C);
    final Color bodyLite = bounce
        ? const Color(0xFF2A6A55)
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(barR, const Radius.circular(3)),
      mgh(
        barR,
    ...
    canvas.drawCircle(
      Offset(w * 0.500, h * 0.326),
      h * 0.024,
      f(const Color(0xFF040404)),
    );
    canvas.drawCircle(
      Offset(w * 0.500, h * 0.326),
      h * 0.013,
      f(const Color(0xFF000000)),
    );
    if (bounce) {
      canvas.drawCircle(
        Offset(w * 0.500, h * 0.326),
    ...
    canvas.drawLine(
      Offset(w * 0.463, h * 0.768),
      Offset(w * 0.458, h * 0.810),
      f(const Color(0xFF555555))..strokeWidth = w * 0.006,
    );
  ...
  void _vmShotgunAim(Canvas canvas, double w, double h) {
    ...
    canvas.drawRRect(
      ...
      mgh(
        pumpR,
        const Color(0xFF3A1A06),
        const Color(0xFF7A4015),
        const Color(0xFF3A1A06),
      ),
    );
    for (var i = 0; i < 5; i++) {
    ...
    canvas.drawLine(
      Offset(w * 0.445, h * 0.650),
      Offset(w * 0.440, h * 0.690),
    ...
    canvas.drawRRect(
      ...
      Paint()
        ..shader = Gradient.linear(
          ...
          Offset(w * 0.692, 0),
          [
            const Color(0xFF3A1606),
            const Color(0xFF7A4010),
            const Color(0xFF3A1606),
          ],
          [0.0, 0.5, 1.0],
        ),
  ...
  void _vmRifleAim(Canvas canvas, double w, double h, {required bool bounce}) {
    ...
    final Color bodyDark = bounce
        ? const Color(0xFF061218)
        : const Color(0xFF141414);
    final Color bodyMid = bounce
        ? const Color(0xFF0C2840)
        : const Color(0xFF303030);
    final Color bodyLite = bounce
        ? const Color(0xFF104068)
        : const Color(0xFF505050);
    ...
    if (!bounce) {
      ...
      canvas.drawRect(
        Rect.fromLTWH(w * 0.310, h * 0.440, w * 0.380, h * 0.006),
        f(const Color(0x33FFFFFF)),
      );
      for (var i = 0; i < 5; i++) {
        canvas.drawLine(
          Offset(w * 0.322 + i * w * 0.060, h * 0.444),
      ...
      canvas.drawRRect(
        RRect.fromRectAndRadius(scopeR, const Radius.circular(6)),
        mgv(
          scopeR,
          const Color(0xFF060E18),
          const Color(0xFF0C2035),
          const Color(0xFF060E18),
        ),
      );
      ...
      canvas.drawLine(
        Offset(w * 0.484, h * 0.452),
        Offset(w * 0.498, h * 0.462),
    ...
    canvas.drawRect(
      Rect.fromLTWH(w * 0.296, h * 0.566, w * 0.408, h * 0.008),
      f(const Color(0x44FFFFFF)),
    );
    ...
    canvas.drawRRect(
      ...
      mgv(
        Rect.fromLTWH(w * 0.410, h * 0.800, w * 0.180, h * 0.250),
        bodyMid,
        bodyLite,
        bodyMid,
      ),
    ...
    canvas.drawLine(
      Offset(w * 0.500, h * 0.812),
      Offset(w * 0.500, h * 1.050),
    ...
    canvas.drawLine(
      Offset(w * 0.494, h * 0.782),
      Offset(w * 0.486, h * 0.826),
    ...
    canvas.drawRRect(
      ...
      mgv(
        Rect.fromLTWH(w * 0.280, h * 0.690, w * 0.150, h * 0.360),
        bodyDark,
        bodyMid,
        bodyDark,
      ),
  ...
  ///
  /// [HIVE MIND OPT]: Uses the shared DijkstraMap visibility grid when
  /// available — O(1) array read instead of O(maxSteps) DDA Raycast.
  /// This eliminates the most expensive per-sprite per-frame CPU cost.
  bool _isSpriteVisible(WorldState state, v64.Vector2 target) {
    // 1. Try the Hive Mind visibility grid first (O(1)).
    final hiveMap = game.worldBloc.dijkstraMap;
    if (hiveMap != null) {
      final x = target.x.floor();
      final y = target.y.floor();
      return hiveMap.isCellVisible(x, y);
    }

    // 2. Fallback: classic DDA Raycast (O(maxSteps)). Only used before the
    // first Dijkstra recompute finishes.
    ...
    while (steps < maxSteps) {
      ...
      if (map.getCell(mapX, mapY).isSolid) return false;
      if (mapX == targetMapX && mapY == targetMapY) return true;
  ...
  void _drawPlasmaOrb(
    ...
    final (cx, depth) = _projectPointToScreenX(
      proj.position,
      screenSize,
      state,
      planeLen,
    ...
    final radius = (screenSize.y / depth * 0.032 * proj.visualScale).clamp(
      3.0,
      40.0,
    );
  ...
  void _drawBulletStreak(
    ...
    final (headX, headDepth) = _projectPointToScreenX(
      proj.position,
      screenSize,
      state,
      planeLen,
    ...
    final centerY = screenSize.y / 2.0;
    final thickness = (screenSize.y / headDepth * 0.012 * proj.visualScale)
        .clamp(1.0, 7.0);
    final glowRadius = (screenSize.y / headDepth * 0.022 * proj.visualScale)
        .clamp(2.5, 16.0);

    // --- Determine tail screen X ---
    const minStreakPx =
        30.0; // always visible even when shooting straight ahead
    final speed = proj.velocity.length;
    double tailX;
    if (speed > 0.1) {
      final tailWorld = proj.position - (proj.velocity / speed) * 0.3;
      final (tx, _) = _projectPointToScreenX(
        tailWorld,
        screenSize,
        state,
        planeLen,
    ...
    final glowColor = proj.isEnemy
        ? const Color(0xCCFF5500)
        : const Color(0xCC00EEFF);
    const coreColor = Color(0xFFFFFFFF);
    ...
    final gradFrom = tailX < headX
        ? Offset(minX, centerY)
        : Offset(maxX, centerY);
    final gradTo = tailX < headX
        ? Offset(maxX, centerY)
        : Offset(minX, centerY);
    canvas.drawRRect(
      ...
      Paint()
        ..shader = Gradient.linear(
          gradFrom,
          gradTo,
          [const Color(0x00000000), glowColor],
        )
        ..blendMode = BlendMode.plus,
    );
  ...
  void _drawBounceSpark(
    ...
    if (alpha <= 0) return;
    final (screenX, depth) = _projectPointToScreenX(
      worldPos,
      screenSize,
      state,
      planeLen,
  ...
  void _drawAmmoPickup(
    ...
    final (sx, depth) = _projectPointToScreenX(
      worldPos,
      screenSize,
      state,
      planeLen,
    ...
    if (pickup.ammoType == AmmoType.bouncing) {
      ...
      canvas.drawRRect(
        RRect.fromRectAndRadius(
          topCapRect,
          Radius.circular(topCapRect.height * 0.4),
        ),
        Paint()
          ..color = const Color(0xFF88FFD0)
          ..blendMode = BlendMode.plus,
      );
      // Outer glow aura
      canvas.drawCircle(
      ...
      canvas.drawRRect(
        ...
        Paint()
          ..shader = Gradient.linear(
            ...
            boxRect.bottomLeft,
            [
              const Color(0xFF8B7A20),
              const Color(0xFFCCA830),
              const Color(0xFF9B8220),
            ],
            [0.0, 0.45, 1.0],
          ),
      );
        ...
        canvas.drawOval(
          ...
          Paint()
            ..shader = Gradient.linear(
              ...
              Offset(bx + bw * 0.04, ty),
              [
                const Color(0xFF7A5C10),
                const Color(0xFFDDAA44),
                const Color(0xFF7A5C10),
              ],
              [0.0, 0.5, 1.0],
            ),
  ...
  void _drawWallDecal(
    ...
    final sparkAlpha = (alpha * 3.0 - 2.0).clamp(
      0.0,
      1.0,
    ); // visible only first 33%
    if (sparkAlpha > 0) {
      canvas.drawCircle(
        Offset(sx, centerY),
        baseR * 0.35,
  ...
  (double, double?) _projectPointToScreenX(
    ...
    final transX = invDet * (_cachedDirY * vec.x - _cachedDirX * vec.y);
    final transY = invDet * (-planeY * vec.x + planeX * vec.y);
